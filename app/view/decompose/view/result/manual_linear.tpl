...
<form id="linears-form" action="/decompose/getExcludedManual" method="post" name="linear_parts">
	<div id="linear-tabs" class="modal-body">
		<ul class="tabs-list">
		{foreach from=$list key="key" item="serie" name="tabs_iterator"}
			<li {if $smarty.foreach.tabs_iterator.first}class="active"{/if}><a href="#linear-tabs-{$key}">{$serie->getCaption()}</a></li>
		{/foreach}
		</ul>
		{foreach from=$list key="key" item="serie" name="tabs_content"}
			<div id="linear-tabs-{$key}" {if $smarty.foreach.tabs_content.first}class="active"{/if}>
